...
void runInference(const std::string& modelPath, const std::string& labelsPath, const std::string& masksPath,
    ...
    for (const auto& imagePath : imageFiles) {
        ...
        std::cout << "Number of segmentations found: " << results.size() << std::endl;
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            std::cout << "Segmentation " << i << ": Class=" << results[i].classId 
                      ...
                      << ", mask shape=(" << results[i].mask.rows << "," << results[i].mask.cols << ")"
                      << std::endl;
        ...
        mask_image_path = masksPath + "/" + model_name + "_" + image_name + "_mask.png";
...
void fromMapToJson(
    const std::unordered_map<std::string, Results>& results, 
    std::string basePath, 
...
int main(int argc, char* argv[]){
    ...
    for(const auto& model : models){
        ...
        runInference(modelPath, labelsPath, masksPath, 
            imageFiles, inferenceConfig, isGPU, 
