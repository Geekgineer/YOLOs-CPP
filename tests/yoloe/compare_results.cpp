...
constexpr double CONF_ERROR_MARGIN = 0.1;
constexpr int BBOX_ERROR_MARGIN = 50;
constexpr double MASK_ERROR_MARGIN = 0.1;
    ...
    for (auto& el : results_ultralytics.items()) {
        ...
        for (size_t i = 0; i < ultra_results.size(); ++i) {
            ...
            for (size_t j = 0; j < cpp_results.size(); ++j) {
                if (cpp_results[j].value("image_path", "") == path_ultra) {
                    path_found = true;
                    break;
                }
            }
                    ...
                    for (size_t idx = 0; idx < segmentations_ultra.size(); ++idx) {
                        ...
                        for (size_t k = 0; k < segmentations_cpp.size(); ++k) {
                            ...
                            if (!already_matched && class_id_ultra == class_id_cpp) {
                                ...
                                if (left_diff <= BBOX_ERROR_MARGIN && top_diff <= BBOX_ERROR_MARGIN &&
                                    width_diff <= BBOX_ERROR_MARGIN && height_diff <= BBOX_ERROR_MARGIN) {
...
TEST_F(ResultsFixture, CompareSegmentationMasks) {
    std::string bp = XSTRING(BASE_PATH_YOLOE);
    for (auto& el : results_ultralytics.items()) {
        const std::string& model_name = el.key();
        auto& ultra_results = el.value()["results"];
        auto& cpp_results = results_cpp[model_name]["results"];
        for (size_t i = 0; i < ultra_results.size(); ++i) {
            std::string image_path = ultra_results[i].value("image_path", "");
            std::string segmentation_mask_ultra = bp + ultra_results[i].value("mask_path", "");
            for (size_t j = 0; j < cpp_results.size(); ++j) {
                if (cpp_results[j].value("image_path", "") == image_path) {
                    std::string segmentation_mask_cpp = bp + cpp_results[j].value("mask_path", "");
