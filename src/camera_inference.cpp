 ...
 * @brief Real-time object detection using YOLO models (v5, v7, v8, v9, v10,
 * v11, v12) with camera input.
 *
 ...
 * versions 5, 7, 8, 9, 10, 11 and 12. The application captures video frames
 * from a specified camera device, processes those frames to detect objects, and
 ...
 * threads:
 * 1. **Producer Thread**: Responsible for capturing frames from the video
 * source and enqueuing them into a thread-safe bounded queue for subsequent
 * processing.
 * 2. **Consumer Thread**: Dequeues frames from the producer's queue, executes
 *    object detection using the specified YOLO model, and enqueues the
 * processed frames along with detection results into another thread-safe
 * bounded queue.
 * 3. **Display Thread**: Dequeues processed frames from the consumer's queue,
 *    draws bounding boxes around detected objects, and displays the frames to
 * the user.
 *
 ...
 *
 * The application employs a double buffering technique by maintaining two
 * bounded queues to efficiently manage the flow of frames between the producer
 * and consumer threads. This setup helps prevent processing delays due to slow
 * frame capture or detection times.
 *
 * Debugging messages can be enabled by defining the `DEBUG_MODE` macro,
 * allowing developers to trace the execution flow and internal state of the
...
int main() {
  ...
  cap.open(videoSource,
           cv::CAP_V4L2);  // Specify V4L2 backend for better performance
  if (!cap.isOpened()) {
  ...
  std::thread producer([&]() {
    ...
    while (!stopFlag.load() && cap.read(frame)) {
      if (!frameQueue.enqueue(frame)) break;  // Queue is finished
    }
