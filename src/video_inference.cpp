 ...
 * @brief Object detection in a video stream using YOLO models (v5, v7, v8, v9,
 * v10, v11, v12).
 *
...
int main() {
  ...
  const std::string videoPath =
      "../data/SIG_experience_center.mp4";  // Input video path
  const std::string outputPath =
      "../data/SIG_experience_center_processed.mp4";  // Output video path
  ...
  int fourcc = static_cast<int>(
      cap.get(cv::CAP_PROP_FOURCC));  // Get codec of input video

  // Create a VideoWriter object to save the output video with the same codec
  cv::VideoWriter out(outputPath, fourcc, fps,
  ...
  std::thread processingThread([&]() {
    ...
    while (frameQueue.dequeue(frame)) {
      ...
      detector.drawBoundingBoxMask(frame,
                                   results);  // Uncomment for mask drawing
