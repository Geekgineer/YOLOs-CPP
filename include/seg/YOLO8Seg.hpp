...
// ====================================================
//
// This header defines the YOLOv8SegDetector class for performing object
// detection and segmentation using the YOLOv8 model. It includes necessary
// libraries, utility structures, and helper functions to facilitate model
...
class YOLOv8SegDetector {
 public:
  YOLOv8SegDetector(const std::string &modelPath, const std::string &labelsPath,
                    bool useGPU = false);
...
inline cv::Mat YOLOv8SegDetector::preprocess(
    const cv::Mat &image, float *&blobPtr,
    std::vector<int64_t> &inputTensorShape) {
...
inline void YOLOv8SegDetector::drawSegmentationsAndBoxes(
...
inline void YOLOv8SegDetector::drawSegmentations(
    cv::Mat &image, const std::vector<Segmentation> &results,
    float maskAlpha) const {
  for (const auto &seg : results) {
    if (seg.conf < CONFIDENCE_THRESHOLD) {
...
inline std::vector<Segmentation> YOLOv8SegDetector::segment(
    const cv::Mat &image, float confThreshold, float iouThreshold) {
