...
// ====================================================
//
// This header defines the YOLOv9SegDetector class for performing Semantic
// segmentation using the YOLOv9 model. It includes necessary libraries, utility
// structures,
// and helper functions to facilitate model inference and result
// post-processing.
//
// Authors:
...
class YOLOv9SegDetector {
 public:
  YOLOv9SegDetector(const std::string &modelPath, const std::string &labelsPath,
                    bool useGPU = false);
...
inline cv::Mat YOLOv9SegDetector::preprocess(
...
inline void YOLOv9SegDetector::drawSegmentationsAndBoxes(
...
inline void YOLOv9SegDetector::drawSegmentations(
    cv::Mat &image, const std::vector<Segmentation> &results,
    float maskAlpha) const {
  for (const auto &seg : results) {
    if (seg.conf < CONFIDENCE_THRESHOLD) {
...
inline std::vector<Segmentation> YOLOv9SegDetector::segment(
