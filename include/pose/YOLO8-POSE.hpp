...
// ===================================
// Single YOLOv8 Pose Detector Header File
// ===================================
//
// This header defines the YOLO8PoseDetector class for performing human pose
// estimation using the YOLOv8 model. It includes necessary libraries, utility
// structures, and helper functions to facilitate model inference, keypoint
// detection, and result visualization.
//
// Authors:
...
/**
 * @file YOLO8-POSE.hpp
 * @brief Header file for the YOLO8PoseDetector class, responsible for human
 * pose estimation using the YOLOv8 model with optimized performance for
...
/**
 * @namespace utils
 * @brief Namespace containing utility functions for the YOLO8POSE-Detector.
 */
namespace utils {
...
inline void drawPoseEstimation(cv::Mat &image,
  ...
  // Original RGB values: [255,128,0], [255,153,51], [255,178,102], [230,230,0],
  // [255,153,255], [153,204,255], [255,102,255], [255,51,255], [102,178,255],
  // [51,153,255], [255,153,153], [255,102,102], [255,51,51], [153,255,153],
  ...
  // Make sure the number of entries here matches the number of pairs in
  // POSE_SKELETON.
  ...
  for (const auto &detection : detections) {
    ...
    // Draw bounding box (optional – remove if you prefer only pose
    // visualization)
    const auto &box = detection.box;
    cv::rectangle(
        image, cv::Point(box.x, box.y),
    ...
    // Draw skeleton connections based on a predefined POSE_SKELETON (vector of
    // pairs) Make sure that POSE_SKELETON is defined with 0-indexed keypoint
    // indices.
...
/**
 * @brief YOLO8POSEDetector class handles loading the YOLO model, preprocessing
 * images, running inference, and postprocessing results.
 */
class YOLO8POSEDetector {
 public:
  /**
   ...
   */
  YOLO8POSEDetector(const std::string &modelPath, const std::string &labelsPath,
                    bool useGPU = false);
  ...
  /**
   * @brief Draws bounding boxes and keypoints (if available) on the provided
   * image.
   *
   * @param image Input/output image on which detections will be visualized.
   * @param detections Vector of detected objects containing bounding boxes and
   * keypoints.
   */
...
// Implementation of YOLO8POSEDetector constructor
YOLO8POSEDetector::YOLO8POSEDetector(const std::string &modelPath,
...
cv::Mat YOLO8POSEDetector::preprocess(const cv::Mat &image, float *&blob,
                                      std::vector<int64_t> &inputTensorShape) {
 ...
 * @param detections Vector of detected objects, including bounding boxes and
 * keypoints.
 *
 * @note Uses `utils::drawPoseEstimation()` for visualization.
 */
void YOLO8POSEDetector::drawBoundingBox(
    cv::Mat &image, const std::vector<Detection> &detections) const {
  utils::drawPoseEstimation(image, detections);
}

/**
 * @brief Processes the raw output tensors from the YOLO model and extracts
 * detections.
 *
 * @param originalImageSize The original size of the input image before
 * resizing.
 ...
 * @param confThreshold Confidence threshold to filter weak detections (default
 * is 0.4).
 * @param iouThreshold IoU threshold for Non-Maximum Suppression (NMS) to remove
 * redundant detections (default is 0.5).
 * @return std::vector<Detection> A vector of final detections after processing.
 *
 * @note This function applies confidence filtering, NMS, and necessary
 * transformations to map detections back to the original image size.
 */
std::vector<Detection> YOLO8POSEDetector::postprocess(
    const cv::Size &originalImageSize, const cv::Size &resizedImageShape,
    const std::vector<Ort::Value> &outputTensors, float confThreshold,
    float iouThreshold) {
...
std::vector<Detection> YOLO8POSEDetector::detect(const cv::Mat &image,
                                                 float confThreshold,
                                                 float iouThreshold) {
