...
// =====================================
//
// This header defines the YOLO11Classifier class for performing image
// classification using an ONNX model. It includes necessary libraries, utility
 ...
 * @brief Header file for the YOLO11Classifier class, responsible for image
 * classification using an ONNX model with optimized performance for minimal
 * latency.
 */
...
struct ClassificationResult {
  int classId{
      -1};  // Predicted class ID, initialized to -1 for easier error checking
...
// ... (clamp, getClassNames, vectorProduct, preprocessImageToTensor,
// drawClassificationResult utilities remain the same as previous correct
// version) ...
/**
...
/**
 * @brief Prepares an image for model input by resizing and padding
 * (letterboxing style) or simple resize.
 */
inline void preprocessImageToTensor(
    const cv::Mat &image, cv::Mat &outImage, const cv::Size &targetShape,
    const cv::Scalar &color = cv::Scalar(0, 0, 0), bool scaleUp = true,
    const std::string &strategy = "resize") {
  if (image.empty()) {
    std::cerr << "ERROR: Input image to preprocessImageToTensor is empty."
              << std::endl;
    return;
  }
 ...
 */
inline void drawClassificationResult(
  ...
  if (image.empty()) {
    std::cerr << "ERROR: Empty image provided to drawClassificationResult."
              << std::endl;
    return;
  }
  if (result.classId == -1) {
...
YOLO11Classifier::YOLO11Classifier(const std::string &modelPath,
  ...
  if (useGPU && cudaAvailable != availableProviders.end()) {
    ...
    if (useGPU) {
      std::cout << "Warning: GPU requested but CUDAExecutionProvider is not "
  ...
  if (modelInputTensorShapeVec.size() == 4) {
    ...
    DEBUG_PRINT("Model input tensor shape from metadata: "
                << modelInputTensorShapeVec[0] << "x"
                << modelInputTensorShapeVec[1] << "x"
                << modelInputTensorShapeVec[2] << "x"
                << modelInputTensorShapeVec[3]);
    ...
    if (!isDynamicInputShape_) {
      ...
      if (modelH != inputImageShape_.height ||
        ...
        std::cout
            << "Warning: Target preprocessing shape ("
            << inputImageShape_.height << "x" << inputImageShape_.width
            << ") differs from model's fixed input shape (" << modelH << "x"
            << modelW << "). "
            << "Image will be preprocessed to " << inputImageShape_.height
            << "x" << inputImageShape_.width << "."
            << " Consider aligning these for optimal performance/accuracy."
            << std::endl;
      }
    } else {
      DEBUG_PRINT(
          "Model has dynamic input H/W. Preprocessing to specified target: "
          << inputImageShape_.height << "x" << inputImageShape_.width);
    }
  } else {
    std::cerr << "Warning: Model input tensor does not have 4 dimensions as "
                 "expected (NCHW). Shape: [";
  ...
  if (numClasses_ > 0) {
    ...
    oss_shape << "]";
    DEBUG_PRINT("Model predicts "
                << numClasses_
                << " classes based on output shape: " << oss_shape.str());
    // END CORRECTED SECTION
  } else {
    std::cerr << "Warning: Could not reliably determine number of classes from "
                 "output shape: [";
    for (size_t i = 0; i < outputTensorShapeVec.size();
         ++i) {  // Directly print to cerr
      std::cerr << outputTensorShapeVec[i]
                << (i == outputTensorShapeVec.size() - 1 ? "" : ", ");
    }
    std::cerr << "]. Postprocessing might be incorrect or assume a default."
              << std::endl;
  }

  classNames_ = utils::getClassNames(labelsPath);
  if (numClasses_ > 0 && !classNames_.empty() &&
      classNames_.size() != static_cast<size_t>(numClasses_)) {
  ...
  if (classNames_.empty() && numClasses_ > 0) {
    std::cout << "Warning: Class names file is empty or failed to load. "
                 "Predictions will use numeric IDs if labels are not available."
              << std::endl;
  }

  std::cout << "YOLO11Classifier initialized successfully. Model: " << modelPath
            << std::endl;
}

// ... (preprocess, postprocess, and classify methods remain the same as
...
void YOLO11Classifier::preprocess(const cv::Mat &image, float *&blob,
  ...
  //    The color cv::Scalar(0,0,0) is for padding if 'letterbox' strategy were
  //    used; unused for 'resize'.
  utils::preprocessImageToTensor(image, processedImage, inputImageShape_,
                                 cv::Scalar(0, 0, 0), true, "resize");

  // 2. Convert BGR (OpenCV default) to RGB
  cv::Mat rgbImageMat;  // Use a different name to avoid confusion if
                        // processedImage was already RGB
  cv::cvtColor(processedImage, rgbImageMat, cv::COLOR_BGR2RGB);

  // 3. Convert to float32. At this point, values are typically in [0, 255]
  // range.
  ...
  if (static_cast<int>(inputTensorShape[2]) != inputImageShape_.height ||
    ...
    std::cerr << "CRITICAL WARNING: Preprocessed image dimensions ("
              << inputTensorShape[2] << "x" << inputTensorShape[3]
              << ") do not match target inputImageShape_ ("
              << inputImageShape_.height << "x" << inputImageShape_.width
              << ") after resizing! This indicates an issue in "
                 "utils::preprocessImageToTensor or logic."
              << std::endl;
  }
  ...
  if (num_channels != 3) {
    ...
    throw std::runtime_error(
        "Expected 3 channels for image blob after RGB conversion, but tensor "
        "shape indicates: " +
  ...
  if (floatRgbImage.channels() != 3) {
    delete[] blob;
    throw std::runtime_error(
        "Expected 3 channels in cv::Mat floatRgbImage, but got: " +
        std::to_string(floatRgbImage.channels()));
  }

  for (int c_idx = 0; c_idx < num_channels;
       ++c_idx) {                    // Iterate over R, G, B channels
    for (int i = 0; i < h; ++i) {    // Iterate over rows (height)
      for (int j = 0; j < w; ++j) {  // Iterate over columns (width)
        // floatRgbImage is HWC (i is row, j is col, c_idx is channel index in
        // Vec3f) floatRgbImage pixel values are float and in [0, 255] range.
  ...
  DEBUG_PRINT(
      "Preprocessing completed (RGB, scaled [0,1]). Actual input tensor shape: "
      << inputTensorShape[0] << "x" << inputTensorShape[1] << "x"
      << inputTensorShape[2] << "x" << inputTensorShape[3]);
}
ClassificationResult YOLO11Classifier::postprocess(
    const std::vector<Ort::Value> &outputTensors) {
  ...
  if (numClasses_ <= 0 && classNames_.empty()) {
    std::cerr << "Error: Number of classes is unknown and no class names "
                 "loaded. Cannot postprocess."
              << std::endl;
    return {};
  }
  ...
  if (currentNumClasses <= 0) {
    std::cerr << "Error: Effective number of classes is zero or negative. "
                 "Cannot determine output scores."
              << std::endl;
    return {};
  }
  ...
  for (size_t i = 0; i < outputShape.size(); ++i)
    oss_shape_debug << outputShape[i]
                    << (i == outputShape.size() - 1 ? "" : ", ");
  oss_shape_debug << "]";
  DEBUG_PRINT("Postprocessing: Output tensor shape = "
              << oss_shape_debug.str() << ", total numScores = " << numScores);
  // === END EXTENDED DEBUGGING ===

  if (outputShape.empty() ||
      !((outputShape.size() == 2 &&
         static_cast<int>(outputShape[1]) == currentNumClasses &&
         outputShape[0] > 0) ||
        (outputShape.size() == 1 &&
         static_cast<int>(outputShape[0]) == currentNumClasses))) {
    std::ostringstream oss_shape_err;
    oss_shape_err << "[";
    for (size_t i = 0; i < outputShape.size(); ++i)
      oss_shape_err << outputShape[i]
                    << (i == outputShape.size() - 1 ? "" : ", ");
    oss_shape_err << "]";
    std::cerr << "Warning: Output tensor shape " << oss_shape_err.str()
              << " does not strictly match expected pattern for "
              << currentNumClasses << " classes. "
              << "Proceeding by assuming the flat tensor of size " << numScores
              << " contains the scores." << std::endl;

    if (static_cast<int>(numScores) != currentNumClasses &&
        outputShape.size() > 1 &&
        static_cast<int>(numScores / outputShape[0]) == currentNumClasses) {
      DEBUG_PRINT(
          "Output tensor seems batched. Using scores from the first batch item "
          "(numScores will be treated as currentNumClasses for the loop).");
      // This case implies numScores for the first item is currentNumClasses.
    } else if (static_cast<int>(numScores) != currentNumClasses) {
      std::cerr << "Error: Total number of scores (" << numScores
                << ") in output tensor does not match number of classes ("
                << currentNumClasses << ")." << std::endl;
      return {};
    }
  }
  ...
  for (int k = 0; k < std::min(10, static_cast<int>(numScores));
       ++k) {  // Use numScores for safety here
  ...
  for (int i = 0; i < currentNumClasses; ++i) {  // Loop up to currentNumClasses
    if (i >= static_cast<int>(
                 numScores)) {  // Boundary check if numScores was smaller than
                                // currentNumClasses (should have been caught)
      std::cerr << "Error: Trying to access rawOutput[" << i
                << "] but numScores is only " << numScores << std::endl;
      break;
    }

    float currentLogit = rawOutput[i];
    if (std::isnan(currentLogit)) {
      // DEBUG_PRINT("Skipping NaN logit at index " << i); // Optional: too
      // verbose if many NaNs
    ...
    if (std::isinf(currentLogit) && currentLogit < 0) {  // -Infinity
      ...
      continue;  // Skip -Infinity as it won't be > maxLogit unless maxLogit is
                 // also -Inf
    }
  ...
  if (bestClassId == -1) {
    std::cerr << "Error: Could not determine best class ID from output scores "
                 "(maxLogit found: "
              << maxLogit
              << "). All valid scores might have been too low or problematic."
              << std::endl;
    return {};
  }

  // ... (rest of the softmax and result creation logic, which is fine if
  // bestClassId is valid) ...
  std::vector<float> probabilities(currentNumClasses);
  float sum_exp_logits = 0.0f;

  // Re-calculate maxLogit from *valid* scores only for stable softmax, if
  // necessary, or use the maxLogit found above if it's not -FLT_MAX (meaning at
  // least one valid score was found). If maxLogit is still -FLT_MAX, it means
  // all scores were NaN or <= -FLT_MAX. The check `if (bestClassId == -1)`
  // should have caught this. So, if we are here, bestClassId is valid and
  // maxLogit is a valid (non -FLT_MAX) number.

  for (int i = 0; i < currentNumClasses; ++i) {
    if (i >= static_cast<int>(numScores))
      break;  // Should not happen if initial checks are okay
    float logit_val = rawOutput[i];
    if (std::isnan(logit_val)) {
      probabilities[i] = 0;  // Treat NaN as zero probability contribution
    } else {
      // Subtracting maxLogit (the actual max *valid* logit) for numerical
      // stability in exp()
  ...
  if (sum_exp_logits > std::numeric_limits<float>::epsilon()) {
    // Get probability of the best class (use rawOutput[bestClassId] for its
    // logit)
    float best_class_logit = rawOutput[bestClassId];
    if (std::isnan(best_class_logit)) {  // Should not happen if bestClassId was
                                         // chosen correctly
      confidence = 0.0f;
      DEBUG_PRINT("Warning: Logit for bestClassId "
                  << bestClassId << " is NaN during confidence calculation.");
    } else {
      confidence = std::exp(best_class_logit - maxLogit) / sum_exp_logits;
    }
  } else {
    confidence =
        (currentNumClasses == 1 && !std::isnan(rawOutput[0])) ? 1.0f : 0.0f;
    DEBUG_PRINT(
        "Warning: Sum of exponentiated logits is near zero during softmax. "
        "Confidence might be inaccurate.");
  }
  ...
  DEBUG_PRINT("Postprocessing completed. Best Class ID: "
              << bestClassId << ", Name: \"" << className << "\", Confidence: "
...
ClassificationResult YOLO11Classifier::classify(const cv::Mat &image) {
  ...
  if (!blobPtr) {
    std::cerr << "Error: Preprocessing failed to produce a valid data blob."
              << std::endl;
    return {};
  }
  ...
  if (inputTensorSize == 0) {
    std::cerr << "Error: Input tensor size is zero after preprocessing."
              << std::endl;
    delete[] blobPtr;
    return {};
  }
  ...
  try {
    ...
    std::cerr << "ONNX Runtime Exception during Run(): " << e.what()
              << std::endl;
    return {};
  }

  if (outputTensors.empty()) {
    std::cerr << "Error: ONNX Runtime Run() produced no output tensors."
              << std::endl;
    return {};
  }
