...
// =====================================
//
// This header defines the YOLO5Classifier class for performing image classification
...
/**
 * @file YOLO5CLASS.hpp
 * @brief Header file for the YOLO5Classifier class, responsible for image classification
...
/**
 * @namespace utils
 * @brief Namespace containing utility functions for the YOLO5Classifier.
 */
namespace utils {
...
/**
 * @brief YOLO5Classifier class handles loading the classification model,
 * preprocessing images, running inference, and postprocessing results.
 */
class YOLO5Classifier {
public:
    /**
     * @brief Constructor to initialize the classifier with model and label paths.
     */
    YOLO5Classifier(const std::string &modelPath, const std::string &labelsPath,
...
// Implementation of YOLO5Classifier constructor
YOLO5Classifier::YOLO5Classifier(const std::string &modelPath, const std::string &labelsPath,
    ...
    std::cout << "YOLO5Classifier initialized successfully. Model: " << modelPath << std::endl;
}

void YOLO5Classifier::preprocess(const cv::Mat &image, float *&blob, std::vector<int64_t> &inputTensorShape) {
...
ClassificationResult YOLO5Classifier::postprocess(const std::vector<Ort::Value> &outputTensors) {
...
ClassificationResult YOLO5Classifier::classify(const cv::Mat& image) {
