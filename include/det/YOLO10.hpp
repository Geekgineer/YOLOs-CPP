...
// ===================================
//
// This header defines the YOLO10Detector class for performing object detection
// using the YOLOv10 model. It includes necessary libraries, utility structures,
 ...
 * @brief Header file for the YOLO10Detector class, responsible for object
 * detection using the YOLOv10 model with optimized performance for minimal
 * latency.
 */
...
inline void drawBoundingBox(cv::Mat &image,
  ...
  for (size_t i = 0; i < numDetections; ++i) {
    ...
    std::string confidenceText =
        std::to_string(static_cast<int>(detection.accuracy * 100)) + "%";
...
inline void drawBoundingBoxMask(cv::Mat &image,
  ...
  for (const auto &detection : detections) {
    if (detection.accuracy > CONFIDENCE_THRESHOLD && detection.obj_id >= 0 &&
        static_cast<size_t>(detection.obj_id) < classNames.size()) {
...
#pragma omp parallel for schedule(dynamic)
  for (size_t i = 0; i < validDetections; ++i) {
    ...
    std::snprintf(labelBuffer, sizeof(labelBuffer), "%s: %.0f%%",
                  classNames[classId].c_str(), detection->accuracy * 100.0f);
...
// YOLO10Detector class handles loading the YOLO model, preprocessing images,
...
class YOLO10Detector {
 public:
   ...
   */
  YOLO10Detector(const std::string &modelPath, const std::string &labelsPath,
                 bool useGPU = false);
...
std::vector<Detection> YOLO10Detector::postprocess(
  ...
  for (int i = 0; i < numDetections; i++) {
    ...
    Detection detection(static_cast<int>(x1), static_cast<int>(x2),
                        static_cast<int>(y1), static_cast<int>(y2), classId,
                        confidence);
