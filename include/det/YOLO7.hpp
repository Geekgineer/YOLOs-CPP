...
// ===================================
//
// This header defines the YOLO7Detector class for performing object detection
// using the YOLOv7 model. It includes necessary libraries, utility structures,
 ...
 * @brief Header file for the YOLO7Detector class, responsible for object
 * detection using the YOLOv7 model with optimized performance for minimal
 * latency.
 */
 ...
 */
inline void scaleResultCoordsToOriginal(const cv::Size &imageShape,
                                        Detection &detection,
                                        const cv::Size &imageOriginalShape) {
  float gain = std::min(static_cast<float>(imageShape.width) /
                            static_cast<float>(imageOriginalShape.width),
                        static_cast<float>(imageShape.height) /
...
inline void drawBoundingBox(cv::Mat &image,
  ...
  for (const auto &detection : detections) {
    if (detection.confidence > CONFIDENCE_THRESHOLD) {
      ...
      cv::rectangle(image,
                    cv::Point(detection.box.x, labelY - textSize.height - 10),
                    cv::Point(detection.box.x + textSize.width + 10,
                              labelY + baseline - 10),
                    color, cv::FILLED);
...
inline std::vector<cv::Scalar> generateColors(
  ...
  if (!colors.empty()) return colors;  // Avoid redundant color generation
...
/**
 * @class YOLO7Detector
 * @brief Class responsible for loading the YOLOv7 model, preprocessing images,
 * running inference, and postprocessing results for object detection with
 * optimized performance.
 */
class YOLO7Detector {
 public:
  /**
   * @brief Constructor to initialize the YOLO7Detector with model and label
   * paths.
   *
   ...
   */
  YOLO7Detector(const std::string &modelPath, const std::string &labelsPath,
                bool isGPU);
  ...
  void drawBoundingBox(cv::Mat &image,
                       const std::vector<Detection> &detectionVector) const {
  ...
  void drawBoundingBoxMask(cv::Mat &image,
                           const std::vector<Detection> &detectionVector,
                           float maskAlpha = 0.4f) const {
    utils::drawBoundingBoxMask(image, detectionVector, classNames, classColors,
                               maskAlpha);
  }
 ...
 */
YOLO7Detector::YOLO7Detector(const std::string &modelPath,
                             const std::string &labelsPath, bool isGPU)
  ...
  std::cout << "YOLO7Detector initialized with " << numInputNodes
 ...
 * @brief Preprocesses the input image for model inference.
 */
cv::Mat YOLO7Detector::preprocess(const cv::Mat &image, float *&blob,
                                  std::vector<int64_t> &inputTensorShape) {
  ...
  // Reuse a pre-allocated buffer if possible (Here, dynamically allocated for
  // simplicity)
  ...
  for (int i = 0; i < resizedImage.channels(); ++i) {
    // Utilize OpenCV's efficient memory sharing to prevent unnecessary data
    // copying
 ...
 * @brief Postprocesses the model output to extract detections.
 */
std::vector<Detection> YOLO7Detector::postprocess(
  ...
  for (int i = 0; i < outputShape[0]; i++) {
    ...
    float accuracy =
        outputShape[1] > 6 ? output[i * outputShape[1] + 6] : confidence;
    ...
    utils::scaleResultCoordsToOriginal(resizedImageShape, detection,
                                       originalImageSize);
...
std::vector<Detection> YOLO7Detector::detect(const cv::Mat &image) {
  ...
  // To minimize latency, move semantics can be used if the blob is no longer
  // needed
