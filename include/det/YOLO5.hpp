...
// ==================================
//
// This header defines the YOLO5Detector class for performing object detection
// using the YOLOv5 model. It includes necessary libraries, utility structures,
 ...
 * @brief Header file for the YOLO5Detector class, responsible for object
 * detection using the YOLOv5 model with optimized performance for minimal
 * latency.
 */
...
struct BoundingBox {
  ...
  /**
   * @brief Parameterized constructor to initialize bounding box coordinates and
   * size.
   *
 ...
 */
inline void NMSBoxes(std::vector<BoundingBox> &boundingBoxes,
                     std::vector<float> &scores, float scoreThreshold,
 ...
 */
inline std::vector<cv::Scalar> generateColors(
  ...
  for (size_t i = 0; i < classNames.size(); ++i) {
    colors.emplace_back(
        cv::Scalar(uni(rng), uni(rng),
                   uni(rng)));  // Generate a unique random color for each class
  }
  return colors;
}
...
/**
 * @brief Draws bounding boxes with semi-transparent masks on the image based on
 * detections.
 *
 * @param image Image on which to draw.
 * @param detections Vector of detections.
...
#pragma omp parallel for schedule(dynamic)
  for (size_t i = 0; i < numDetections; ++i) {
    ...
    // Define top-left and bottom-right points for the label background
    // rectangle
    cv::Point labelTopLeft(detection.box.x,
                           labelY - labelSize.height - baseLine);
    cv::Point labelBottomRight(detection.box.x + labelSize.width,
                               labelY + baseLine - baseLine);

    // Draw filled rectangle behind the label for better visibility
    cv::rectangle(image, labelTopLeft, labelBottomRight, color, cv::FILLED);

    // Put the label text on the image
    cv::putText(image, label, cv::Point(detection.box.x, labelY),
...
/**
 * @brief YOLO5Detector class handles loading the YOLOv5 model, preprocessing
 * images, running inference, and postprocessing results to perform object
 * detection.
 */
class YOLO5Detector {
 public:
  /**
   * @brief Constructor to initialize the YOLO5Detector with model and label
   * paths.
   *
   ...
   */
  YOLO5Detector(const std::string &modelPath, const std::string &labelsPath,
                const bool &isGPU);
  ...
  /**
   * @brief Draws bounding boxes with semi-transparent masks on the image based
   * on detections.
   *
   * @param image Image on which to draw.
   * @param detections Vector of detections.
 ...
 private:
  ...
  Ort::Session session{
      nullptr}; /**< ONNX Runtime session for running inference */
  ...
  std::vector<std::string>
      inputNamesString; /**< Allocated input node names (for ORT API >= 13) */
  std::vector<std::string>
      outputNamesString; /**< Allocated output node names (for ORT API >= 13) */
#endif

  std::vector<std::vector<int64_t>>
      input_node_dims; /**< Dimensions of input nodes */
  std::vector<std::vector<int64_t>>
      output_node_dims;     /**< Dimensions of output nodes */
   ...
   * shape.
   */
  void preprocess(const cv::Mat &image, std::unique_ptr<float[]> &blob,
 ...
 *
 * Initializes the ONNX Runtime environment, loads the model, sets up session
 * options, and prepares class names and colors.
 *
 * @param modelPath Path to the ONNX model file.
 * @param labelsPath Path to the file containing class labels.
 * @param isGPU Flag indicating whether to use GPU for inference.
 */
YOLO5Detector::YOLO5Detector(const std::string &modelPath,
                             const std::string &labelsPath, const bool &isGPU) {
 ...
 * @param allocator Allocator for handling memory.
 */
void YOLO5Detector::getInputDetails(
    Ort::AllocatorWithDefaultOptions &allocator) {
  for (int layer = 0; layer < this->session.GetInputCount(); ++layer) {
    ...
    Ort::AllocatedStringPtr input_name_Ptr =
        this->session.GetInputNameAllocated(layer, allocator);
    inputNamesString.push_back(input_name_Ptr.get());
    inputNames.push_back(inputNamesString[layer].c_str());
#endif

    std::vector<int64_t> inputTensorShape =
        this->session.GetInputTypeInfo(layer)
            .GetTensorTypeAndShapeInfo()
            .GetShape();
    if (inputTensorShape.size() >= 4 && inputTensorShape[2] == -1 &&
 ...
 * @param allocator Allocator for handling memory.
 */
void YOLO5Detector::getOutputDetails(
    Ort::AllocatorWithDefaultOptions &allocator) {
  for (int layer = 0; layer < this->session.GetOutputCount(); ++layer) {
    ...
    auto outputTensorShape = this->session.GetOutputTypeInfo(layer)
                                 .GetTensorTypeAndShapeInfo()
                                 .GetShape();
 ...
 */
void YOLO5Detector::preprocess(const cv::Mat &image,
  ...
  blob = std::make_unique<float[]>(resizedImage.cols * resizedImage.rows *
  ...
  for (int c = 0; c < 3; ++c) {
    for (int h = 0; h < resizedImage.rows; ++h) {
      for (int w = 0; w < resizedImage.cols; ++w) {
        blob[c * resizedImage.cols * resizedImage.rows + h * resizedImage.cols +
             w] = resizedImage.at<cv::Vec3f>(h, w)[c];
      }
    }
  }
 ...
 * @return std::vector<Detection> Vector of detection results.
 */
std::vector<Detection> YOLO5Detector::detect(cv::Mat &image,
                                             const float &confThreshold,
                                             const float &iouThreshold) {
  ...
  std::vector<float> inputTensorValues(blob.get(),
                                       blob.get() + inputTensorSize);
  ...
  std::vector<Detection> result = this->postprocessing(
      resizedShape, image.size(), outputTensors, confThreshold, iouThreshold);
 ...
 * @param detections Vector of detections.
 */
void YOLO5Detector::drawBoundingBox(cv::Mat &image,
                                    const std::vector<Detection> &detections) {
 ...
 */
void YOLO5Detector::drawBoundingBoxMask(
 ...
 * @return std::vector<Detection> Vector of detection results.
 */
std::vector<Detection> YOLO5Detector::postprocessing(
  ...
  for (auto it = output.begin(); it != output.begin() + elementsInBatch;
    ...
    if (clsConf > confThreshold) {
      ...
      BoundingBox scaledBox =
          utils::scaleCoords(resizedImageShape, box, originalImageShape, true);
  ...
  utils::NMSBoxes(boundingBoxes, scores, confThreshold, iouThreshold,
                  nmsIndices);
 ...
 * @param bestClassId Reference to store the best class ID.
 */
void YOLO5Detector::getBestClassInfo(std::vector<float>::iterator it,
                                     const int &numClasses, float &bestConf,
                                     int &bestClassId) {
  bestClassId = 5;
  bestConf = 0.0f;
