...
// ===================================
//
// This header defines the YOLO8Detector class for performing object detection
// using the YOLOv8 model. It includes necessary libraries, utility structures,
 ...
 * @brief Header file for the YOLO8Detector class, responsible for object
 * detection using the YOLOv8 model with optimized performance for minimal
 * latency.
 */
...
/**
 * @brief YOLO8Detector class handles loading the YOLO model, preprocessing
...
class YOLO8Detector {
 public:
   ...
   */
  YOLO8Detector(const std::string &modelPath, const std::string &labelsPath,
                bool useGPU = false);
...
YOLO8Detector::YOLO8Detector(const std::string &modelPath,
...
cv::Mat YOLO8Detector::preprocess(const cv::Mat &image, float *&blob,
                                  std::vector<int64_t> &inputTensorShape) {
...
std::vector<Detection> YOLO8Detector::postprocess(
  ...
  for (size_t d = 0; d < num_detections; ++d) {
    ...
    if (objConf > confThreshold) {
      ...
      // Round coordinates and adjust for class ID to prevent NMS overlap
      // between classes
  ...
  for (const int idx : indices) {
    detections.emplace_back(Detection{boxes[idx], confs[idx], classIds[idx]});
  }
...
void YOLO8Detector::getBestClassInfo(const std::vector<float> &p_Mat,
  ...
  for (int i = 0; i < numClasses; ++i) {
    float classConf =
        p_Mat[i + 4];  // Assuming first 4 elements are bbox coordinates
...
std::vector<Detection> YOLO8Detector::detect(const cv::Mat &image,
                                             float confThreshold,
                                             float iouThreshold) {
