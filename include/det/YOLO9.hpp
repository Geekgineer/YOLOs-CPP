...
// ===================================
//
// This header defines the YOLO9Detector class for performing object detection
 ...
 * @brief Header file for the YOLO9Detector class, responsible for object
 * detection using the YOLOv9 model with optimized performance for minimal
 * latency.
 */
...
inline void drawBoundingBox(cv::Mat &image,
  ...
  for (size_t i = 0; i < numDetections; ++i) {
    ...
    if (detection.confidence <= CONFIDENCE_THRESHOLD) continue;
    ...
    cv::Size confidenceSize = cv::getTextSize(confidenceText, fontFace,
    ...
    cv::Point confidenceOrg(detection.x1,
                            detection.y2 + confidenceSize.height + 10);
    ...
    cv::rectangle(image, textOrg + cv::Point(0, baseline),
                  textOrg + cv::Point(textSize.width, -textSize.height), color,
                  cv::FILLED);

    cv::rectangle(
        image, confidenceOrg + cv::Point(0, baseline),
 ...
 */
inline const std::vector<cv::Scalar> &generateColors(
...
inline void drawBoundingBoxMask(cv::Mat &image,
  ...
  // and with valid class IDs This reduces the workload in the parallel section
...
#pragma omp parallel for schedule(dynamic)
  for (size_t i = 0; i < validDetections; ++i) {
    ...
    // Draw the bounding box on the original image with anti-aliased lines for
    // smoothness
    cv::rectangle(image, box, color, 2, cv::LINE_AA);

    // Prepare the label with class name and confidence percentage
    // Preallocate a buffer for the label string to avoid dynamic memory
    // allocation
    char labelBuffer[256];
    std::snprintf(labelBuffer, sizeof(labelBuffer), "%s: %.0f%%",
                  classNames[classId].c_str(), detection->confidence * 100.0f);
    ...
    // Define the top-left and bottom-right points for the label background
    // rectangle
    ...
    cv::putText(image, label, cv::Point(x1 + 2, labelY - 2),
...
// YOLO9Detector class handles loading the YOLO model, preprocessing images,
...
class YOLO9Detector {
 public:
   ...
   */
  YOLO9Detector(const std::string &modelPath, const std::string &labelsPath,
                bool useGPU = false);
...
YOLO9Detector::YOLO9Detector(const std::string &modelPath,
  ...
  sessionOptions.SetIntraOpNumThreads(
      std::min(6, (int)std::thread::hardware_concurrency()));
  ...
  isDynamicInputShape =
      ...
      inputTensorShape[3] == -1;  // Check for dynamic dimensions
...
cv::Mat YOLO9Detector::preprocess(const cv::Mat &image, float *&blob,
                                  std::vector<int64_t> &inputTensorShape) {
...
std::vector<Detection> YOLO9Detector::postprocess(
  ...
  for (int i = 0; i < numPredictions; ++i) {
    ...
    const float height =
        rawOutput[i + 3 * numPredictions];  // Height at stride 3
    ...
    Detection detection(static_cast<int>(x_center - width / 2),   // x1
  ...
  for (const auto &det : detectionVector) {
    boxes.emplace_back(
        cv::Rect(det.x1, det.y1, det.x2 - det.x1, det.y2 - det.y1));
