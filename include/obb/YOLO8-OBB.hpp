...
// ===================================
//
// This header defines the YOLO8-OBB-Detector class for performing object
// detection using the YOLOv8-OBB model. It includes necessary libraries,
// utility structures, and helper functions to facilitate model inference and
 ...
 * @brief Header file for the YOLO8OBBDetector class, responsible for object
 * detection using the YOLOv8 OBB model with optimized performance for minimal
 * latency.
 */
 ...
 */
void getCovarianceComponents(const OrientedBoundingBox &box, float &out1,
 ...
 */
std::vector<std::vector<float>> batchProbiou(
    const std::vector<OrientedBoundingBox> &obb1,
    const std::vector<OrientedBoundingBox> &obb2, float eps = EPS) {
  size_t N = obb1.size();
  size_t M = obb2.size();
 ...
 */
std::vector<int> nmsRotatedImpl(
    const std::vector<OrientedBoundingBox> &sorted_boxes, float iou_thres) {
...
std::vector<Detection> nonMaxSuppression(
    const std::vector<Detection> &input_detections, float conf_thres = 0.25f,
    float iou_thres = 0.75f, int max_det = 1000) {
...
/**
 * @brief Draws oriented bounding boxes with rotation and labels on the image
 * based on detections
 *
...
/**
 * @brief YOLO8-OBB-Detector class handles loading the YOLO model, preprocessing
...
class YOLO8OBBDetector {
 public:
   ...
   */
  YOLO8OBBDetector(const std::string &modelPath, const std::string &labelsPath,
                   bool useGPU = false);
 ...
 private:
  ...
  /**
   * @brief Postprocesses the model output to extract detections with oriented
   * bounding boxes.
   *
   ...
   * @param iouThreshold IoU threshold for Non-Maximum Suppression (using
   * ProbIoU for rotated boxes).
   * @return std::vector<Detection> Vector of detections with oriented bounding
   * boxes.
   */
...
YOLO8OBBDetector::YOLO8OBBDetector(const std::string &modelPath,
...
cv::Mat YOLO8OBBDetector::preprocess(const cv::Mat &image, float *&blob,
                                     std::vector<int64_t> &inputTensorShape) {
...
std::vector<Detection> YOLO8OBBDetector::detect(const cv::Mat &image,
                                                float confThreshold,
                                                float iouThreshold) {
