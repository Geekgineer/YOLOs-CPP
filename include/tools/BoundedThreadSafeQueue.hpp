...
class BoundedThreadSafeQueue {
 public:
  BoundedThreadSafeQueue(size_t max_size)
      : max_size_(max_size), finished(false) {
    DEBUG_PRINT(
        "BoundedThreadSafeQueue initialized with max size: " << max_size_);
  }

  bool enqueue(T item) {
    std::unique_lock<std::mutex> lock(m);
    cv_not_full.wait(lock,
                     [this]() { return q.size() < max_size_ || finished; });
